...
let rec pairs values = match values with
| s :: e :: xs -> (s, e) :: pairs xs
| [] -> []
| _ -> failwith "Unreachable"

let expand (start, length) = 
    List.init length (fun x -> x + start)

let () = 
    let lines = read_lines () in
    let (seeds, lines) = parse_seeds lines in
    let p = pairs seeds in
    let seeds = List.flatten (List.map expand p) in
